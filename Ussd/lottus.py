...
class Lottus(object):
    ...
    def process_request(self, request):
        ...
        if(window['name'] == ''):
            print('Not windows in process request')
            ...
            if 'window' in required:
                if 'in_options' in required and required['in_options'] == True:
                    ...
                    if selected_option:
                        ...
                        print('selected_option 1#')
                        print('SM do else')
                        validationResult = self.is_valid_arbitary_input(actual_window, request)
                        print(validationResult)
                        if validationResult["valid"]:
                            window, session = self.get_mapped_window(validationResult['window'], session, request)
                        else:
                            actual_window['message'] = "Selecione uma opcao valida"
                            window = actual_window
                else:
                    #print('selected_option Emilio')
            ...
            if selected_option is None:
                ...
                if selected_option['window'] in self.mapped_windows:
                    ...
                    if(selected_option['action'] == ''):
                        ...
                        #print('rv')
                        rv = getattr(clientactions, selected_option['action'])(request['phone'],request['command'])
                        #print(rv)
                        window, session = self.get_mapped_window(selected_option['window'], session, request)
                    ...
                    if(selected_option['action'] == ''):
                        ...
                        rv = getattr(clientactions, selected_option['action'])(request['phone'],request['command'])
                        #print(rv)
                        if(selected_option['window'] ==  ''):
                            print('OK no windows')
                            print(selected_option['window'])
                            window = rv
                        else:
                            window = self.get_window(selected_option['window'])
                        #print('in Lottus')
    ...
    def is_valid_arbitary_input(self, window, request):
        ...
        for s in options:
            if s['option'] == "-1":
                ...
                if(len(s['action'])>0):
                    novoValor = getattr(clientactions, s['action'])(request['phone'],request['command'])
                    nextWindow['valid'] = novoValor['valid']
...
def create_option(option, display, window, action="", active=True):
    """
...
def create_error_window(message):
    ...
    """
    return create_window(name='ERROR', message=message, title='title', window_type='MESSAGE')
